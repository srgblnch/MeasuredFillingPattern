        ...
        try:
            ...
            for i,name in enumerate(ynames):
                ...
                if not self.curves.has_key(name):
                    curve = StreamingCurve(name, xname, self,
...
class StreamingCurve(TaurusCurve):
    def __init__(self,name,xname=None,parent=None,rawData=None,optimized=False):
        super(StreamingCurve,self).__init__(name,xname,parent,rawData,optimized)
    ...
    def eventReceived(self, evt_src, evt_type, evt_value):
        ...
        if hasattr(self,'_queueLock'):
            ...
            self.warning("%s receive an event, but no locker, processing the "\
                         "event without the streaming feature"%(evt_src.name))
